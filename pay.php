...
$orderTotal = filter_input(INPUT_GET, 'order_total', FILTER_VALIDATE_FLOAT);
...
if (!$orderTotal || $orderTotal <= 0) {
    die(json_encode(['success' => false, 'error' => 'Invalid Order Total']));
}

// Verify order exists and is pending
try {
    $stmt = $pdo->prepare("SELECT * FROM pos_order WHERE order_id = ? AND order_status = 'Pending'");
...
<head>
    ...
    <title>Complete Order Payment</title>
                    ...
                    <table class="table table-sm">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th><?= $confData['currency'] . number_format($orderTotal, 2) ?></th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <form id="paymentForm">
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="amount_received" 
                           ...
                           min="<?= number_format($orderTotal, 2, '.', '') ?>" 
                           step="0.01" required>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            paymentModeSelect.addEventListener('change', function() {
                if (this.value === 'Cash') {
                    ...
                    amountReceivedInput.value = <?= number_format($orderTotal, 2, '.', '') ?>;
                }
            });
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!amountReceived || amountReceived <= 0) {
                    ...
                    isValid = false;
                } else if (paymentModeSelect.value === 'Cash' && amountReceived < <?= number_format($orderTotal, 2, '.', '') ?>) {
                    document.getElementById('amountReceivedError').textContent = 'Amount received must be at least the order total';
                    isValid = false;
                }
